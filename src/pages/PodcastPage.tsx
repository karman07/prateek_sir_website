...
const PodcastPage: React.FC = () => {
  ...
  // Extract unique places for filter options
  const uniquePlaces = [...new Set(uniquePodcasts.map((p) => p.place))];

  // Helper: extract year from podcast.date
  const extractYear = (dateStr: string): number => {
    const match = dateStr.match(/\b(19|20)\d{2}\b/); // find 4-digit year
            ...
            filteredAndSortedPodcasts.map((podcast, index) => (
              ...
              >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <p className="text-slate-600 text-sm font-medium">{podcast.date}</p>
                </div>
